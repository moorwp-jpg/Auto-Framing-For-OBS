...
bool obs_module_load(void) {
    ...
    blog(LOG_INFO, "[obs-auto-framing] loaded (version %s%s%s)", PLUGIN_VERSION, release_channel[0] != '\0' ? " " : "",
         release_channel);
    autoframing::register_auto_framing_filter();
    return true;
}

void obs_module_unload(void) { blog(LOG_INFO, "[obs-auto-framing] unloaded"); }
